...
int main(void) {
  ...
  k_msleep(120); // The software reset command can take up to 120 ms before the next
                 // command can be processed
  lcd_cmd(CMD_SLEEP_OUT, NULL);
  lcd_cmd(CMD_DISPLAY_ON, NULL);
  //640x480
  int square_size = 10*10*3;
  uint8_t column_data[] = {0x00, 149, 0x00, 159}; //Column 149 to 159
  uint8_t row_data[] = {0x00, 109, 0x00, 119}; // Row 117 to 127
  uint8_t color_data[square_size];
  for (int i = 0; i < square_size; i+=3) {
    color_data[i] = 0; //blue
    color_data[i+1] = 0xFC; //green
    color_data[i+2] = 0; //red
  }
  struct spi_buf column_data_buf = {column_data, 4};
  struct spi_buf row_data_buf = {row_data, 4};
  struct spi_buf color_data_buf = {color_data, square_size};
